...
function parseDocument(realtimeDoc, requestedLocales) {
  const messages = [ ];
  ...
  let localeTargets;
  let invalidSpecified = false;

  if (requestedLocales == null || requestedLocales == "*") {
    localeTargets = locales.all.map((locale) => ({ locale, ref: { } }));
  } else {
    localeTargets = castToArray(requestedLocales)
      .map((requestedCode) => {
        const locale = locales.getByCode(requestedCode);

        if (!locale) {
          messages.push(`No locale with code '${requestedCode}'.`);
  ...
  if (localeTargets.length === 0) {
    messages.push("Nothing to do - no valid locales specified.");
  }

  if (invalidSpecified) {
    messages.push(
      `Document contains ${locales.all.length} locale(s): ` +
      formatLocaleArray(locales.all)
    );
  }
